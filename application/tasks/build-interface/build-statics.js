...
let buildStatic = (() => {
	var _ref = _asyncToGenerator(function* (pkg, target) {
		...
		yield _sander2.default.copydir(assetSourcePath).to(assetTargetPath);
		yield _sander2.default.copydir(clientStaticSourcePath).to(clientStaticTargetPath);

		if (yield (0, _pathExists2.default)(staticSourcePath)) {
			yield _sander2.default.copydir(staticSourcePath).to(staticTargetPath);
		}
	});
